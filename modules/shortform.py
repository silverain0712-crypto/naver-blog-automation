...
[motion_worth — 신중하게 판정하라]
움직임이 '정보나 설득'을 만드는 컷만 high 다. 한 영상에 최대 2개까지만 high 를 준다.
- high 예: 물에 뜬 방수 스피커(물결), 김이 나는 음식, 아이가 만지는 장난감, 흐르는 액체
- low 예: 제품 스펙샷, 구성품 나열, 표, 패키지 정면, 텍스트가 중요한 화면
애매하면 low 다. high 를 남발하면 비용만 오르고 영상은 산만해진다.
motion_prompt 는 영어 한 문장으로, 카메라가 아니라 '피사체가 어떻게 움직이는지'를 쓴다.
