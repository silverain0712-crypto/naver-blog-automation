...
def generate_post(
    *,
    ...
    system = (
        ...
        "네이버 상위노출 글은 공통적으로 제목 앞에 '세부 차별 키워드'를 박는다 — 브랜드·모델명, "
        "제품이면 용량/사이즈/형태(예: 12L·오븐형·집게형·문어발형), 육아 정보면 개월수(예: 15개월), "
        "장소·시즌이면 연도(예: 2026)와 세부 실용질문(예: '물 나오는 시간', '주차 꿀팁'). "
        "후킹어는 후기·추천·비교·내돈내산·총정리·완벽정리 중 맥락에 맞게. 공백 제외 25~35자, 낚시·과장 금지.\n"
        "- 관련 키워드 2개가 한 세트로 검색되면 제목에 나란히 병기하라(예: '아기 칫솔 유아 칫솔', "
        "'보관함 정리함'). 한 글로 두 검색어에 동시 노출된다.\n"
        "- 도입부(첫 2~3문장)에서 독자의 상황·고민을 특정하라. 상위노출 도입부는 셋 중 하나다 — "
        "(a)페인포인트 공감형('~때문에 늘 골치였어요'), (b)결론 선제형('결론부터 말하면 ~가 다 되더라구요'), "
        "(c)계절·날씨 훅(시즌·장소 글). 여기에 핵심 키워드를 자연스럽게 3회 넣어라.\n"
        "- 정보 밀도로 승부하라(상위노출·체류시간의 핵심). 제품은 스펙·수치·성분·인증·비교, "
        "육아 정보는 개월수·기간 수치와 단계별 방법론(가능하면 방법 명칭), "
        ...
        + video_rule
        + "\n\n[출력 형식]\n"
        "- suggested_keywords: 이 글의 추천 핵심 키워드 3~5개. 단일 대형 키워드보다 "
        ...
        "**정확히 3칸(3열)**으로 맞춰라(첫 행은 머리글, 데이터는 2행 이상). 비교·스펙·요금·시간 정보가 있으면 "
